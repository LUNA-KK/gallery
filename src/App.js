...
function useEventListener(eventName, handler, element = document) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      element.removeEventListener(eventName, eventListener); // 이 부분이 올바르게 설정되어야 함
    };
  }, [eventName, element]);
}
function AnimatedCursor({
  ...
  const onMouseMove = React.useCallback(({ clientX, clientY }) => {
    if (!cursorInnerRef.current || !cursorOuterRef.current) return; // 방어 코드 추가
  ...
  React.useEffect(() => {
    requestRef.current = requestAnimationFrame(animateOuterCursor);
    return () => cancelAnimationFrame(requestRef.current); // cleanup 함수 추가
  }, [animateOuterCursor]);
